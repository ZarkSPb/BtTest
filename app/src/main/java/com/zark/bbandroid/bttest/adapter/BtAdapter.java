...
import com.zark.bbandroid.bttest.R;
...
public class BtAdapter extends ArrayAdapter<ListItem> {
   ...
   private void savePrefs(int pos) {
   ...
   private View defaultItem(View convertView, int position, ViewGroup parent) {
      ...
      if (pref.getString(BtConsts.MAC_KEY,
            "no bt selected").equals(mainList.get(position).getBtDevice().getAddress())) {
         viewHolder.chBtSelected.setChecked(true);
      }
